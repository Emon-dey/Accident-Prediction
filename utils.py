...
def roc_auc_compute(city,y_test,y_score,n_classes=2):
    fpr = dict()
    tpr = dict()
    roc_auc = dict()
...
def plot_roc(city,fpr,tpr,roc_auc,n_classes=2):
    lw = 2
# Plot all ROC curves
    plt.figure(1)
    ...
    plt.title('ROC For '+ city)
    plt.savefig('a_{}.png'.format(city), bbox_inches='tight', dpi=1200)
    plt.show()
    ...
    plt.legend(loc="lower right")
    plt.title('ROC For '+ city)
    plt.savefig('b_{}.png'.format(city), bbox_inches='tight', dpi=1200)
    plt.show()
