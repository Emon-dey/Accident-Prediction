...
class base_model(object):        
    ...
    def load_data(self,category=None,with_geocode=False):
        self.X_train = np.load('X_train_'+self.CITY+'.npy')
        self.y_train = np.load('y_train_'+self.CITY+'.npy')
        self.X_test = np.load('X_test_'+self.CITY+'.npy')
        self.y_test = np.load('y_test_'+self.CITY+'.npy')
    ...
    def last_layers(self,model_in):
        model_in = Conv1D(32,3,
                    kernel_regularizer=regularizers.l2(self.weight_decay),
                    activation=self.act, input_shape=(model_in.shape))(model_in)
#         model_in = MaxPooling1D(pool_size=3,strides=None, padding="valid")
...
class keras_model(base_model):
    def __init__(self,city='Atlanta',activation='softmax',batch_size=128, epoch =40 ,n_jobs=1,act=ACT_POSTERIOR):
    ...
    def confusion(self,city,y_true,y_pred):
        data_frame1 = confusion_matrix(y_true.argmax(axis=-1), y_pred.argmax(axis=-1))
    ...
    def roc_auc(self,city,y_test,y_score):
        fpr,tpr,roc_auc = roc_auc_compute(city,y_test,y_score)
        plot_roc(city,fpr,tpr,roc_auc)
        return roc_auc
